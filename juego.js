...
var instrucciones = ["Mover las imagenes con las flechas",
    "Como un Rompecabezas, ordenar las imagenes hasta alcanzar el Objectivo del juego",
    "Guiarse pro la imagen abajo expuesta"
];
// Arreglo para ir guardando los movimientos que se vayan realizando
var movimientos = [];

//Variable cantidad de veces mezcladas
var mezclaVeces = 50;
...
function chequearSiGano() {
    ...
    for (let row = 0; row < grilla.length; row++) {
        for (let column = 0; column < grilla[row].length; column++) {
            var valorActual = grilla[row][column];
            if (valorActual === grillaCorrecta[row][column]) {
                counter++;
            }
        }
    }
...
function mostrarCartelGanador() {
    console.log("Adentro de mostrar Cartel Ganador");
    shake("#juego div");
    setTimeout(function() {
...
function moverEnDireccion(direccion) {
    ...
    // Mueve pieza hacia la izquierda, reemplazandola con la negra
...
function intercambiarPosiciones(fila1, columna1, fila2, columna2) {
    ...
    //Intercambio en el array "Grilla"
    intercambiarPosicionesGrilla(fila1, columna1, fila2, columna2);

    //Intercambio en el html las posiciones
...
presiona el usuario */
function iniciar() {
    mezclarPiezas(mezclaVeces);
    movimientos = [];
    capturarTeclas();
}

// Arranque automático para carga de instrucciones
mostrarInstrucciones(instrucciones);

//Tiembla el objeto insertado
function shake(elementCSSmarker) {
    $(elementCSSmarker).effect("shake");
};